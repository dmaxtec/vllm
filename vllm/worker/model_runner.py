...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def prepare_model_input(
        ...
        # For each sequence group, count the prompt tokens that represent 
        # placeholders for the multimodal data.
        seq_multimodal_tokens = (
            torch.tensor(
                [
                    next(
                        iter(seq_group_metadata.seq_data.values())
                    ).prompt_token_ids.count(
